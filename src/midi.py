...
class MidiKey:
    def __init__(self, key_id, white_key_container=None, black_key_container=None):
        """A MidiKey is a class representing a key on a piano and its pitch. Its `init()` function calculates its
        information given its `key_id`. """
        ...
        self.init_sprite()
        print("Initialized key " + str(self.key_id) + " with pitch " + str(self.pitch) + " and name " + str(
            self.key_name))
        if white_key_container is not None and black_key_container is not None:
            self.add_to_rendering(white_key_container, black_key_container)

    def get_key_name(self):
        """Get the MidiKey's `key_name` tuple. This will later be indexed based on whether the key has sharps or
        flats. """
        key_names = [("C", "C"), ("C#", "Db"), ("D", "D"), ("D#", "Eb"), ("E", "E"), ("F", "F"), ("F#", "Gb"),
                     ("G", "G"), ("G#", "Ab"), ("A", "A"), ("A#", "Bb"), ("B", "B")]
    ...
    def add_to_rendering(self, white_key_container, black_key_container):
        """Add the MidiKey as a piano key to the `white_key_container` and `black_key_container`. This is done by
        checking the `key_color` of the key. """
